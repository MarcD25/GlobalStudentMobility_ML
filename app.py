...
with tab4:
    ...
    with st.container(border=True):
        ...
        1.  **Country Profiling Reveals Diverse Mobility Categories:**
            Through K-Means and Hierarchical Clustering, we moved beyond simple leaderboards to identify distinct country profiles. Our analysis reveals groups such as "High-Inbound Hubs" (countries that attract many international students), "Primarily Senders" (countries with higher outbound rates), and "Balanced Mobility" countries with moderate flows in both directions, alongside a substantial group of "Low Mobility" countries with minimal international student flows. The geographical distribution shows clear regional patterns, with certain clusters concentrated in specific world regions, providing a nuanced understanding of the global education landscape.

        2.  **Classification Models Show Promise with Key Limitations:**
            The machine learning approach to predict mobility categories using multiple algorithms revealed encouraging results. Random Forest achieved the highest accuracy for inbound mobility prediction, while Logistic Regression performed best for outbound mobility classification. However, both models struggled with the "Medium" mobility category, showing higher accuracy for extreme categories (High/Low). The analysis revealed that economic indicators (GDP per capita, education spending) are the strongest predictors, followed by digital infrastructure (internet access) and political stability. Notably, most misclassifications occurred between adjacent categories, suggesting the models capture the general mobility hierarchy well but struggle with boundary cases.

        3.  **Feature Importance Unveils the Drivers of Global Mobility:**
            The predictive modeling consistently identified key factors that determine a country's mobility profile. Economic capacity emerges as the primary driver - countries with higher GDP per capita and greater education spending per capita are more likely to participate actively in international student exchange. Digital connectivity plays a surprisingly significant role, reflecting how internet access facilitates international education opportunities and applications. For inbound mobility specifically, political stability becomes crucial, as students and families seek confidence in destination countries. This hierarchy of factors provides a roadmap for countries seeking to improve their position in the global education landscape, suggesting that economic development, digital infrastructure, and political stability form the foundation for successful international student mobility.
        """)
